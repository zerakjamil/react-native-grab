...
import { clearGrabOwnerPresentation, hideGrabSelectionMenu, registerLocalGrabSelectionController, setGrabSelectionSessionOwner, showGrabSelectionMenu, stopGrabFreeze, unregisterLocalGrabSelectionController, useGrabControllerState, } from "./grab-controller";
...
export const ReactNativeGrabOverlay = ({ ownerId, onPanHandlersChange, }) => {
    ...
    const getElementAtPoint = useCallback((pageX, pageY) => {
        ...
        onPanResponderRelease: (evt) => {
            void (async () => {
                try {
                    ...
                    if (!result) {
                        if (freezeStateRef.current.isActive &&
    ...
    return (_jsxs(View, { pointerEvents: overlayPointerEvents, style: styles.overlayRoot, ...(overlayTouchHandlers ?? {}), onLayout: (event) => {
            ...
            setBounds((prev) => {
                ...
                return { width, height };
        ...
        }, children: [freezeState.isActive && freezeState.snapshot?.uri && (_jsx(View, { pointerEvents: "none", style: styles.freezeImageWrap, children: _jsx(Image, { resizeMode: "cover", source: { uri: freezeState.snapshot.uri }, style: styles.freezeImage }) })), statusBadgeText && (_jsx(View, { pointerEvents: "none", style: [styles.topBadge, freezeState.error && styles.topBadgeError], children: _jsx(Text, { style: styles.topBadgeText, children: statusBadgeText }) })), !!highlightedElement && (_jsx(View, { pointerEvents: "none", style: [
                    styles.highlight,
                    {
                ...
                ] })), showLayoutMetrics && layoutMetrics && (_jsx(View, { pointerEvents: "none", style: [
                    styles.metricsBadge,
                    {
                        left: Math.min(Math.max(8, layoutMetrics.x), Math.max(8, bounds.width - 160)),
                        top: Math.max(8, layoutMetrics.y - 28),
                    },
                ], children: _jsx(Text, { style: styles.metricsText, children: `x:${layoutMetrics.x} y:${layoutMetrics.y} w:${layoutMetrics.width} h:${layoutMetrics.height}` }) })), _jsxs(ContextMenu, { anchor: selectedElementMenuAnchor, bounds: bounds, cutout: state.selectedElement
                    ? {
                    ...
                    : null, horizontalAlignment: "center", offset: { x: 0, y: 8 }, onClose: closeSelectedElementMenu, visible: state.selectedElement !== null, children: [_jsx(View, { style: styles.selectionMenuHeader, children: _jsx(Text, { adjustsFontSizeToFit: true, minimumFontScale: 0.65, numberOfLines: 1, style: styles.selectionMenuTitle, children: state.selectedElement?.elementName }) }), _jsx(ContextMenu.Item, { onPress: () => void handleCopySelectedElement(), children: "Copy summary" }), _jsx(ContextMenu.Item, { onPress: () => void handleCopyElementPath(), children: "Copy name + path" }), _jsx(ContextMenu.Item, { onPress: () => void handleCopyProps(), children: "Copy props JSON" }), _jsx(ContextMenu.Item, { onPress: handleToggleLayoutMetrics, children: showLayoutMetrics ? "Hide layout metrics" : "Show layout metrics" }), _jsx(ContextMenu.Item, { disabled: !state.selectedElement?.frame?.file, onPress: () => void handleOpenSelectedElement(), children: "Open in editor" }), freezeState.isActive && (_jsx(ContextMenu.Item, { destructive: true, onPress: handleUnfreeze, children: "Unfreeze" }))] })] }));
};
